...
public class TripleInputRecipe extends MachineRecipe {
    ...
    public boolean matches(Container inv, Level level) {
        ...
        var matchedContainerItems = new Ingredient[inv.getContainerSize() - MachineInventory.NON_INPUT_SLOTS];
        Set<Ingredient> matchedIngredients = new HashSet<>();

        for (var slot = MachineInventory.NON_INPUT_SLOTS; slot < inv.getContainerSize(); slot++) {
            var stack = inv.getItem(slot);
            if (!stack.isEmpty() && matchedContainerItems[slot - MachineInventory.NON_INPUT_SLOTS] == null) {
                for (var input : inputs) {
                    if (!matchedIngredients.contains(input) && input.test(stack)) {
                        matchedContainerItems[slot - MachineInventory.NON_INPUT_SLOTS] = input;
